		...
		initPage: function(){
			...
			this.gridOption = {
					cols : [
						...
						, {title:"차단 수", name:"SCOUNT", colWidth:"7"}
						, 
						{title : "수정", name: "modify", colWidth:"5", filter:function(data, rowData, ridx, cidx, $this) {
							return `<a class="btn btn-outline-primary btn-sm" href="/parkingBlock/parkingBlockRevise?BLO_SEQ=${rowData.BLO_SEQ}">수정</a>`
						}}
						, 
						{title : "삭제", name: "BSEQ", colWidth:"5", filter:function(data, rowData, ridx, cidx, $this) {
							return `<a class="btn btn-outline-danger btn-sm" onclick="parkingBlock.deleteRow(${ridx})">삭제</a>`
						}}
						],
